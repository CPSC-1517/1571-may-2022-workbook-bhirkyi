    ...
    public class EmploymentTests
    {
        ...
        public void Employment_ToStringDisplay_GoodDisplay(string title, SupervisoryLevel level, double years)
        {
            try
            {
                ...
                //Act
                string displaypfToString = employment.ToString();

                //Assess
                Assert.AreEqual(displaypfToString, "Unit Test Designer,Owner,25.2", $"ToString {employment.ToString()} not as expected.");
        ...
        public void Employment_SetSupervisoryLevel_GoodSet(SupervisoryLevel level)
        {
                ...
                //arrange (set up of the test/data)
            ...
            //assess (checking for success)
            Assert.IsTrue(employment.Level == level, $"Emplyment level of {employment.Level} is incorrect it should be {level} ");
